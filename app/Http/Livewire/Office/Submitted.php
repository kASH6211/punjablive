...
{   use WithPagination;
    ...
    public function render()
    {
        ...
        $header=['Office','Department','Sub-department', 'Office Address'];
            ...
            $data =  OfficeMaster::where('distcode',$this->distcode)->whereIn('id', OfficeLock::get('office_id'))->when($this->search,function($query, $search){
                return $query->where('office','ILIKE',"%$this->search%");
            })->orderBy('id','DESC')->paginate($this->perPage);
            $data->withPath('/office/detail');
            $tot =  OfficeMaster::where('distcode',$this->distcode)->whereIn('id', OfficeLock::get('office_id'))->get()->count();

        return view('livewire.office.submitted',["data"=>$data,"header"=>$header,"total"=>$tot]);
    }
