...
class DistrictData extends Component
{
    ...
    {
        $this->hpdcount  = DB::table('hrms_polling_data')->count();
        return view('livewire.import.hrms.district-data');
    }
...
protected $rules=[
    ...
    public function import()
    {
       ...
       // $this->validate();
        ...
        // $excelfile = SimpleExcelReader::open($path);
        ...
        $pd_ram=PollingData::get();
        
        foreach($pd_ram  as $p)
        {
         $p->delete();  //Soft Deleting existing records
        }
       dispatch(new \App\Jobs\HRMSPollingDataImportJob());
        // $employees=DB::table('hrms_polling_data')->get();
        // $employees=SimpleExcelReader::create($this->file->getRealPath())
        // ->fromSheet(5)
        // ->trimHeaderRow()
        // ->headerOnRow(0)
        // ->getRows();
       ...
       //  $this->addEmployees($employees);
        // for($i=0;$i<100;$i++)
        // $test=PollingData::get();
         
        //   gc_collect_cycles();
        //   $endmemory=memory_get_usage();
        //   dd('main',$endmemory-$startmemory);
    ...
    public function addEmployees($employees)
    {
       ...
       $testmem=0;
        ...
        $t1=now();
        //dd($employees);
        $employees->each(function ($row)   use(&$testmem){
        ...
        //     $t2=now();
        //         $diffInSeconds = $t2->diff($t1)->s;
        //  $microsecondsPart = ($t2->diff($t1)->f + $diffInSeconds) * 1000000;
        //  dd('payscalecode',$microsecondsPart);
            ...
            //dd($distcode,$deptcode,$officecode,$deptslno);
            ...
            if($distcode!=-1 and $distcode!=null and $deptcode!=null and $officecode!=null and $officecode!=null and $deptslno!=null)
            {
                ...
                // $dob =  Carbon::createFromDate(1900, 1, 1)->addDays($row->DOB - 2);
                // $dor =  Carbon::createFromDate(1900, 1, 1)->addDays($row->DOR - 2);
                $t1=now();
        ...
        //         {
        //             OfficeMaster::where('distcode',$dist->distcode)->where('hrmsdata',1)->where('deptcode',$dept->deptcode)->whereNotIn('officecode',PollingData::where('distcode',$dist->distcode)->where('hrmsdata',1)->where('deptcode',$dept->deptcode)->get('officecode'))->delete();
        //         }
        //     }
  
      PollingData::onlyTrashed()->restore();

    //   OfficeMaster::onlyTrashed()->forceDelete();
    //   HrmsOffice::onlyTrashed()->forceDelete();

    //   DesignationMaster::onlyTrashed()->forceDelete();
    //   HrmsDesignation::onlyTrashed()->forceDelete();

    //   DeptMaster::onlyTrashed()->forceDelete();
    //   HrmsDepartment::onlyTrashed()->forceDelete();
